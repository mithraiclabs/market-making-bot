...
async def derive_option_key_from_params(
    expiration: float,
    quote_asset_mint: float,
    program_id: PublicKey,
    underlying_asset_mint: float,
    underlying_asset_per_contract: float,
    quote_asset_per_contract: float,
):
...
async def find_open_orders_accounts_for_owner(
    program: Program, dex_program_id: PublicKey, serum_market_address: PublicKey
):
    instructions = [
        b"open-orders",
    ...
    results: typing.Tuple[PublicKey, int] = PublicKey.find_program_address(
        instructions, program.program_id
    )
...
async def main():
    ...
    results = await find_open_orders_accounts_for_owner(
        program, dex_program_id, serum_market_address
    )
    print("finished")
